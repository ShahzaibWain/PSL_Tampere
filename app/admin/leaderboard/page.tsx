...
export default function AdminLeaderboardPage() {
  ...
  const exportSelectedTeam = () => {
    ...
    const headers = ['Sr', 'Team', 'Player Name', 'Category', 'Bought Price', 'Bought Price Words']
    const csvLines = [
      headers.join(','),
      ...rows.map((row) =>
        [
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <p className="text-lg font-bold text-slate-900">
                    {team.playersBought} / {team.max_players}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-slate-900">
                    {formatMoneyWords(team.budget_remaining)}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-slate-900">
                    {formatMoneyWords(team.totalSpent)}
                  </p>
                </div>
              </div>
            </button>
        ...
        {selectedTeam ? (
          <div className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div className="flex items-center gap-4">
                <img
                  src={selectedTeam.logo_url || '/team-logos/psl.png'}
                  alt={selectedTeam.name}
                  className="h-20 w-20 rounded-full bg-slate-100 object-contain p-2"
                />
                <div>
                  <h2 className="text-3xl font-bold text-slate-900">{selectedTeam.name} Squad</h2>
                  <p className="text-slate-500">
                    {selectedTeam.playersBought} players bought • {selectedTeam.remainingSlots} slots left
                  </p>
                </div>
              </div>
                ...
                <div className="grid gap-3 text-right sm:grid-cols-3">
                  <div>
                    ...
                    <p className="text-xl font-bold text-slate-900">
                      {formatMoneyWords(selectedTeam.budget_remaining)}
                    </p>
                  </div>
                  ...
                  <div>
                    <p className="text-sm text-slate-500">Total Spent</p>
                    <p className="text-xl font-bold text-slate-900">
                      {formatMoneyWords(selectedTeam.totalSpent)}
                    </p>
                  </div>
                  ...
                  <div>
                    <p className="text-sm text-slate-500">Players Bought</p>
                    <p className="text-xl font-bold text-slate-900">
                      {selectedTeam.playersBought} / {selectedTeam.max_players}
                    </p>
                  </div>
                </div>

                <button
                  onClick={exportSelectedTeam}
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-slate-500">
                          {item.players?.category || 'Unknown Category'}
                        </p>
