...
export default function AdminAuctionPage() {
  ...
  const loadAll = async () => {
    const { data: openPoolPlayers } = await supabase.from('players').select('*').eq('status', 'unsold').order('queue_order', { ascending: true })
    setPlayers(openPoolPlayers || [])
          ...
          {auctionState?.current_player_id && currentPlayer ? (
            <>
              ...
              <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
                Admin protections: Sold is disabled without a leading bid. Unsold and Send To End are only for no-bid cases. Risky actions now use SQL RPC functions for safer updates. The open player pool now uses only the unsold status.
              </div>
